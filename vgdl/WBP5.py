		...
		for k in list(rle._game.sprite_groups.keys()):
			for o in rle._game.sprite_groups[k]:
				if o not in rle._game.kill_list:
					...
					vecValue = pos[1] + pos[0]*rle.outdim[0]
					lst[o.ID] = vecValue
					# lst.append((o, vec))
				else:
					lst[o.ID] = 'gone'
	...
	def novelty(self, node, k, update=False):
		...
		if len(self.noveltyDict) > 0:
			...
			for i in range(1,k+1):
				...
				flattened = [(u[0]+u[1]) for u in unflattened]	
		...
		newTuples = []
		candidatesDict = [dict([(k,node.state[k]) for k in c]) for c in candidates]
		for c in candidatesDict:
			if c not in self.noveltyDict:
				newTuples.append(c)
				if update:
					self.noveltyDict.append(c)
		# node.candidates = candidates
		# newTuples, newAtoms = set(), set()
		# for c in candidates:
		# 	if not all([node.state[el] in self.trueAtoms[el] for el in c]):
		# 		for k in c:
		# 			if node.state[k] not in self.trueAtoms[k]:
		# 				newAtoms.add(k) # present value of k needs to be added to historical values for that element
		# 		newTuples.add(c)
		# print 'in novelty'
		# print len(newTuples)
		# embed()
		# if update:
		# 	for k in newAtoms:
		# 		self.trueAtoms[k].add(node.state[k])
		# embed()
...
def BFS(rle, WBP, k):
	...
	while not Q.empty():
		...
		# if len(current.lastState._game.kill_list) == 1:
			# embed()
